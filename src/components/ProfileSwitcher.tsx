...
import { AddProfileDialog } from '../screens/AddProfileDialog';

export const ProfileSwitcher: React.FC = () => {
  const { profiles, activeProfileId, setActiveProfile, isPremium } = useStore();
  ...
  return (
    <>
      <DropdownMenu>
        ...
        <DropdownMenuContent align="start" className="w-56 rounded-2xl p-2 shadow-xl border-slate-100">
          ...
          <DropdownMenuItem 
            ...
            className="flex items-center gap-3 p-2 rounded-xl cursor-pointer hover:bg-indigo-50 text-indigo-600 transition-colors"
          >
      ...
      <AddProfileDialog open={showAddDialog} onClose={() => setShowAddDialog(false)} />
    </>
  );
};
